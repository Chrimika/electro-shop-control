...
import { db, collection, query, orderBy, getDocs, doc, updateDoc } from '../../lib/firebase';
...
const OwnerSalesPage = () => {
    ...
    if (window.confirm('Êtes-vous sûr de vouloir annuler cette vente ?')) {
      try {
        await updateDoc(doc(db, 'sales', id), {
          status: 'cancelled',
          cancellationReason: 'Annulée par le propriétaire',
  ...
  const filteredSales = sales.filter(sale => {
    ...
    return matchesSearch && matchesStatus && matchesType && matchesStore && matchesDate;
  });
  
  const getStoreName = (storeId: string) => {
    const store = stores.find(s => s.id === storeId);
    return store ? store.name : storeId.substring(0, 6);
  };
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center mb-6">
          ...
          <Button onClick={() => navigate('/owner/sales/new')}>
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Gérer les ventes de toutes les boutiques</CardDescription>
          </CardHeader>
                ...
                <Table>
                  ...
                  <TableBody>
                    {filteredSales.map((sale) => (
                      <TableRow key={sale.id} className="cursor-pointer hover:bg-gray-50" onClick={() => navigate(`/owner/sales/${sale.id}`)}>
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={(e) => {
                                e.stopPropagation();
                                navigate(`/owner/sales/${sale.id}`);
                              }}>
                                <Eye className="h-4 w-4 mr-2" /> Voir détails
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={(e) => {
                                e.stopPropagation();
                                window.open(`/owner/sales/${sale.id}/print`, '_blank');
                              }}>
